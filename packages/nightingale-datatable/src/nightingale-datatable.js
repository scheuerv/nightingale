...
class NightingaleDatatable extends LitElement {
  ...
  connectedCallback() {
    ...
    this.addEventListener("load", (e) => {
      if (Array.from(this.children).includes(e.target)) {
        this.data = NightingaleDatatable.processData(e.detail.payload.features);
      }
    });
    ...
    // this makes sure the nightingale-zoomable event listener doesn't reset
  ...
  set data(value) {
    ...
    this._data = NightingaleDatatable.processData(value);
  ...
  eventHandler(e) {
    if (
      !e.target.closest("nightingale-datatable") &&
  ...
  getStyleClass(id, start, end) {
    ...
    if (
      this.displayStart &&
      this.displayEnd &&
      NightingaleDatatable.isOutside(
        this.displayStart,
        this.displayEnd,
        start,
        end
    ...
    if (
      this.highlight &&
      NightingaleDatatable.isWithinRange(
        this.highlight[0],
        this.highlight[1],
        start,
        end
      ...
      <tr class="child-row">
        ...
        >
          ${childRowItems.map((column) => {
            ...
            return data
              ? html`
                  <div class="nightingale-datatable__child-item">
                    <div class="nightingale-datatable__child-item__title">
                      ${this.columns[column].label}
                    </div>
                    <div class="nightingale-datatable__child-item__content">
  ...
  render() {
    ...
    return html`
      <div
        class="nightingale-datatable-container"
...
export default NightingaleDatatable;
